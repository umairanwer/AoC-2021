...
for(let [index, line] of fileInput.entries()){    //entries function returns an iterator for an array
    ...
    if(x1 !== x2 && y1 !== y2){  //for diagonal lines
        let xAx = x1;
        let yAx = y1;
        let xInc = (x2-x1) > 0 ? 1 : -1;   //wheter to increment or decrement on every loop
        let yInc = (y2-y1) > 0 ? 1 : -1;
        while(xAx != x2 && yAx != y2){
            //console.log(x1, y1, x2, y2, xAx, yAx, xInc, yInc)

            grid[yAx][xAx]++;
            xAx += xInc;
            yAx += yInc;
        }
        grid[y2][x2]++; //because these values are not reached in while 
        continue;
    }
